...
export default class AlterTableQuery extends TableQueryDefinition {
  ...
  private alterColumns(): string[][] {
    const toAdd = this.columnsToAdd.map((col) =>
      col.buildToAdd(this.tableName),
    );
    const toAlter = Array.from(this.columnsToAlter.entries()).flatMap(
      ([colName, alterations]) => {
        return alterations.map((alteration) =>
          alteration.buildToAlter(this.tableName, colName),
        );
      },
    );
  ...
  private dropColumns(): string[] {
    return this.columnsToDrop.map((colName) =>
      AlterTableQuery.dropColumn(colName),
    );
  }
  ...
  public build(_deepAnalysis?: boolean): string[] {
    ...
    dropCols.forEach((dropPart) => {
      const query = `ALTER TABLE ${this.tableName} ${dropPart}`;
      queries.push(query);
    });
    ...
    queries.forEach((query) => {
      (this.builtQuery as any as string[])?.push(
        SqlEscaper.appendSchemas(`${query};`, this.schemas),
      );
    });
