...
export default class Union extends DmlQueryDefinition {
  ...
  public rawUnion(): { text: string; values: any[] } {
    ...
    for (const { query, type } of this.selectQueries) {
      ...
      const builtQuery = query.addWhereOffset(paramOffset - 1).build();
  ...
  public add(query: SelectQuery, type: UnionType = "UNION ALL"): Union {
    ...
    if (!unionTypesArray.includes(type))
      throw new Error(
        "Invalid union type. Only 'UNION' and 'UNION ALL' are allowed.",
      );
   ...
   * @returns The current Union instance for method chaining.
   */
  public addManyOfType(
    queries: SelectQuery[],
    type: UnionType = "UNION ALL",
  ): Union {
  ...
  public build(deepAnalysis: boolean = false): { text: string; values: any[] } {
    ...
    const firstLine = `SELECT${selectClause.length > 1 ? "\n" : ""} ${selectClause}${selectClause.length > 1 ? "\n" : ""} FROM (`;
  ...
  public clone(): Union {
    ...
    newUnion.havingStatement = this.havingStatement
      ? this.havingStatement.clone()
      : null;
    newUnion.whereStatement = this.whereStatement
