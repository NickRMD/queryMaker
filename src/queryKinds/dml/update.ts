  ...
  public set(values: SetValue | SetValue[] | { [key: string]: any }): this {
    if (Array.isArray(values)) {
      this.setValues = values
        ...
        .map((v) => ({
          setColumn: v.setColumn
            ? SqlEscaper.escapeTableName(v.setColumn, this.flavor)
            : "",
          from: v.from
            ? SqlEscaper.escapeTableName(v.from, this.flavor)
            : (undefined as any),
          value: v.value ?? null,
        }));
    } else if (
      values?.setColumn &&
      (values?.from || values?.value !== undefined)
    ) {
      this.setValues = [
        {
          setColumn: values.setColumn
            ? SqlEscaper.escapeTableName(values.setColumn, this.flavor)
            : "",
          from: values.from
            ? SqlEscaper.escapeTableName(values.from, this.flavor)
            : (undefined as any),
          value: values.value ?? null,
        },
      ];
    } else if (typeof values === "object" && !Array.isArray(values)) {
      this.setValues = Object.entries(values)
        .filter(([, val]) => {
  ...
  public build(deepAnalysis: boolean = false): { text: string; values: any[] } {
    ...
    this.setValues.forEach((sv) => {
      if (sv.value !== undefined) {
        ...
        throw new Error(
          `SET value for column ${sv.setColumn} must have either 'value' or 'from' defined.`,
        );
      }
    });
